...
export default function PrintCenterPage({ notify }) {
  ...
  const openManifestForm = (manifest = null) => {
    setEditingManifest(manifest)
    setManifestForm(manifest ? { name: manifest.name, manifest_date: manifest.manifest_date } : { name: '', manifest_date: today })
    setStructureError('')
  ...
  const saveManifest = async (event) => {
    ...
    try {
      const saved = await api(editingManifest ? `/manifests/${editingManifest.id}` : '/manifests', { method: editingManifest ? 'PATCH' : 'POST', body: JSON.stringify(manifestForm) })
      const wasEditing = Boolean(editingManifest)
      setManifestOpen(false); setEditingManifest(null); setManifestForm({ name: '', manifest_date: today })
      if (wasEditing) {
        await Promise.all([loadManifests(saved.id), loadShipments(selectedBagId)])
        notify('Manifiesto actualizado.')
      } else {
        await loadManifests(saved.id); setSelectedManifestId(saved.id); setEditingBag(null); setBagForm({ number: 1, name: '', attendant: '' }); setBagOpen(true)
    ...
    <main id="main-content" className="workspace-content">
      {section === 'addresses' ? <AddressesView notify={notify} /> : section === 'manifests' ? (
        <div className="screen-content">
          <div className="section-heading"><div><span className="section-kicker">DOCUMENTOS</span><h1>Manifiestos y maletas</h1><p>Crea manifiestos, agrega sus maletas y revisa todas sus hojas antes de imprimir.</p></div><div className="section-heading__actions">{selectedManifest && <button className="button button--secondary" onClick={() => openManifestForm(selectedManifest)}><Pencil /> Editar manifiesto</button>}<button className="button button--primary" onClick={() => openManifestForm()}><FolderPlus /> Nuevo manifiesto</button></div></div>
          {hierarchyControls}
          {!selectedManifestId ? <EmptyHierarchy type="manifest" onAction={() => openManifestForm()} /> : !selectedBagId ? <EmptyHierarchy type="bag" onAction={openBagForm} /> : <>
            <section className="manifest-actions"><div><strong>{selectedManifest?.name}</strong><span>{selectedManifest?.manifest_date} · {bags.length} maletas · {selectedManifest?.voucher_count || 0} bauchers</span></div><div><button className="button button--secondary" onClick={() => openBagForm(selectedBag)}><Pencil /> Editar maleta</button><button className="button button--secondary" disabled={!bags.length} onClick={openExpandedManifest}><Expand /> Ver manifiesto en grande</button><button className="button button--secondary" disabled={!shipments.length} onClick={() => printManifest([selectedBag])}><Printer /> Imprimir esta maleta</button><button className="button button--secondary" disabled={!bags.length} onClick={printAllManifestVouchers}><TicketCheck /> Imprimir todos los bauchers</button><button className="button button--primary" disabled={!bags.length} onClick={() => printManifest()}><FileSpreadsheet /> Imprimir manifiesto completo</button></div></section>
