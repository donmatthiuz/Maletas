...
export default function PrintCenterPage({ notify }) {
  ...
  const createManifest = async (event) => {
    ...
    try {
      ...
      setManifestOpen(false); setManifestForm({ name: '', manifest_date: today, attendant: 'DORIAN SANTIZO' })
      await loadManifests(created.id); setSelectedManifestId(created.id); setBagForm({ number: 1, name: '' }); setBagOpen(true)
  ...
  const openBagForm = () => {
    setBagForm({ number: Math.max(0, ...bags.map((bag) => bag.number)) + 1, name: '' })
    setStructureError(''); setBagOpen(true)
  }
  const createBag = async (event) => {
    event.preventDefault(); setSavingStructure(true); setStructureError('')
    try {
      const created = await api(`/manifests/${selectedManifestId}/bags`, { method: 'POST', body: JSON.stringify({ ...bagForm, number: Number(bagForm.number), name: bagForm.name || null }) })
      setBagOpen(false); await loadBags(selectedManifestId, created.id); setSelectedBagId(created.id)
  ...
  return <div className="workspace-shell">
    ...
    <main id="main-content" className="workspace-content">
      ...
      <div className="print-only manifest-print-root">{manifestPrintSheets.map(({ bag, shipments: bagShipments, pageIndex }, index) => <ExcelManifest key={`${bag.id}-${pageIndex}-${index}`} shipments={bagShipments} bagNumber={bag.number} manifestDate={selectedManifest?.manifest_date} attendant={selectedManifest?.attendant} batch />)}</div>
